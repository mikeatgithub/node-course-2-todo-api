...
describe('Get /todos/:id', () => {
    ...
    it('Should not return todo doccreated by another user', (done) => {   // Since async we need done() callback
         request(app)
    ...
    it('Should return 404 1f todo not found.', (done) => {
        // let id = new ObjectID();
        request(app)
            .get(`/todos/${new ObjectID().toHexString()}`)  // Create a new object ID
            .set('x-auth', users[0].tokens[0].token)
            .expect(404)
            .end(done);
    });
    ...
    it('Should reject invalid login', (done) => {
        ...
        .send({
            ...
            password: users[1].password + 
                    'screw up the password to fail the test'
        })
        .expect(400)
        .expect((response) => {
            // Cannot use headers.x-auth form.
